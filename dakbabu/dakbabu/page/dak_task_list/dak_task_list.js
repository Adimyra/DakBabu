	...
	frappe.call({
		...
		callback: function (r) {
			if (r.message) {
				...
				frappe.pages["dak_task_list"].toggle_view(
					frappe.pages["dak_task_list"].current_view
				);
				...
				if (r.message && r.message.length > 0) {
					...
					let activeTask = r.message.find((t) => t.custom_working_now == 1);
...
frappe.pages["dak_task_list"].render_tasks_visuals = function (wrapper, tasks) {
	...
	frappe.pages["dak_task_list"].set_working_task = function (task_name) {
		...
		if (task && task.status === "Completed") {
			frappe.msgprint({
				...
				message: __(
					"This task is already <b>Completed</b>. Please re-open it before setting it as active."
				),
				indicator: "red",
			});
			return;
		...
		frappe.call({
			...
			callback: function (r) {
				if (r.message) {
					...
					// Refresh
					frappe.pages["dak_task_list"].render_task_list(
						frappe.pages["dak_task_list"].page_wrapper
					);
				}
			},
		});
	...
	if (frappe.pages["dak_task_list"].visible_columns) {
		let hasWorking = frappe.pages["dak_task_list"].visible_columns.find(
			(c) => c.field === "custom_working_now"
		);
		if (!hasWorking) {
			frappe.pages["dak_task_list"].visible_columns.unshift({
				label: "Working",
				field: "custom_working_now",
				width: "10%",
			});
		}
	}
	...
	frappe.pages["dak_task_list"].visible_columns.forEach((col, index) => {
		if (col.field === "custom_working_now") return; // Skip if handled manually
	...
	const render_pagination_controls = (totalTasks) => {
		...
		let controlsHtml = `
            <div style="display: flex; justify-content: space-between; align-items: center; padding: 15px; background: #fff; border-top: 1px solid #e5e7eb; margin-top: 10px; border-radius: 0 0 8px 8px;">
                ...
                <div style="display: flex; gap: 5px;">
                    <button class="btn btn-default btn-xs" id="prev-page" ${
						pagination.page === 1 ? "disabled" : ""
					} style="padding: 4px 8px;"><i class="fa fa-chevron-left"></i></button>
                    <button class="btn btn-default btn-xs" id="next-page" ${
						pagination.page === totalPages ? "disabled" : ""
					} style="padding: 4px 8px;"><i class="fa fa-chevron-right"></i></button>
                </div>
            </div>`;
		...
		if ($("#pagination-controls").length === 0) {
			...
			if ($("#pagination-controls").length === 0)
				$("#standard-task-list").append('<div id="pagination-controls"></div>');
		}
	...
	if (tasks && tasks.length > 0) {
		...
		const render_group = (groupName, groupTasks) => {
			...
			if (groupBy) {
				table_html += `<tr class="group-header" style="background:#f9fafb;"><td colspan="${
					frappe.pages["dak_task_list"].visible_columns.length + 2
				}" style="font-weight:700; padding:10px 15px; border-bottom:2px solid #e5e7eb;">${groupName} (${
					groupTasks.length
				})</td></tr>`;
			}
			groupTasks.forEach((task) => {
		...
		if (groupBy) {
			...
			Object.keys(groups)
				.sort()
				.forEach((key) => render_group(key, groups[key]));
		} else {
			// Card View
			html += `<div style="display: flex; flex-wrap: wrap; gap: 30px; width: 100%;">`;
		...
		kanban_cols.forEach((groupVal) => {
			...
			statusTasks.forEach((task) => {
				kanban_html += frappe.pages["dak_task_list"].get_kanban_card_html(
					task,
					kanban_current_field
				);
			});
			kanban_html += `</div></div>`;
		});
		...
		container.html(
			'<div style="text-align: center; padding: 40px; color: rgba(0,0,0,0.5);">No tasks found matching criteria.</div>'
		);
		table_body.html(
			`<tr><td colspan="100%" style="text-align: center; padding: 20px; color: #6b7280;">No tasks found matching criteria.</td></tr>`
		);
		$("#kanban-columns-wrapper").html(
			'<div style="text-align: center; padding: 40px; color: rgba(0,0,0,0.5); width: 100%;">No tasks found.</div>'
		);
		frappe.pages["dak_task_list"].toggle_view(frappe.pages["dak_task_list"].current_view);
	}
};
...
frappe.pages["dak_task_list"].get_task_card_html = function (task) {
	...
	let dateDisplay = task.exp_end_date
    ...
    " onclick="frappe.pages['dak_task_list'].show_task_details('${task.name}')"
		...
		task.status
                ...
                <span style="background: ${priorityColor.bg}; color: ${
		priorityColor.text
	}; padding: 4px 12px; border-radius: 20px; font-size: 0.8rem; font-weight: 600;">${
		task.priority
	}</span>
            ...
            <h3 style="font-size: 1.4rem; font-weight: 700; color: #ffffff; margin-bottom: 8px;">${
				task.subject
			}</h3>
            <p style="color: rgba(255,255,255,0.9); font-size: 0.95rem; margin: 0; line-height: 1.4; display: -webkit-box; -webkit-line-clamp: 2; -webkit-box-orient: vertical; overflow: hidden;">${
...
frappe.pages["dak_task_list"].get_task_row_html = function (task) {
	...
	tr += `
        <td style="text-align: center; vertical-align: middle;" onclick="event.stopPropagation(); frappe.pages['dak_task_list'].set_working_task('${
			task.name
		}')">
            <div style="cursor: pointer; padding: 5px;" title="${
				isWorking ? "Currently Working" : "Set as Working"
			}">
                ${workingIcon}
            </div>
        </td>
	...
	frappe.pages["dak_task_list"].visible_columns.forEach((col) => {
		...
		if (col.field === "subject") {
			...
			cellContent = `<span class="priority-text ${frappe.pages[
				"dak_task_list"
			].getPriorityClass(task.priority)}">${task.priority}</span>`;
		} else if (col.field === "exp_end_date") {
			let dateClass = "due-date";
			if (task.status === "Overdue") dateClass += " overdue";
			cellContent = `<span class="${dateClass}">${
	...
	let onClick = isCompleted
		? ""
		: `onclick="frappe.pages['dak_task_list'].mark_task_complete(event, '${task.name}')"`;
	// Only add hover effect if not completed
	let onHover = isCompleted
		? ""
		: `onmouseover="this.style.color='#10b981'" onmouseout="this.style.color='#d1d5db'"`;
	...
	frappe.confirm(`Are you sure you want to mark task <b>${task_name}</b> as Completed?`, () => {
		...
		frappe.call({
			...
			callback: function (r) {
				if (!r.exc) {
					frappe.show_alert({
						message: __("Task Marked Completed"),
						indicator: "green",
					});
					// Optimistic Update
					let t = frappe.pages["dak_task_list"].all_tasks.find(
						(x) => x.name === task_name
					);
					if (t) {
						t.status = "Completed";
...
frappe.pages["dak_task_list"].get_kanban_card_html = function (task, group_field) {
    ...
    style="border-left-color: ${borderColor};">
        <div style="font-weight: 600; font-size: 0.95rem; margin-bottom: 5px; color: #1f2937;">${
			task.subject
		}</div>
...
frappe.pages["dak_task_list"].show_task_details = function (task_name) {
	...
	let task = frappe.pages["dak_task_list"].all_tasks
		? frappe.pages["dak_task_list"].all_tasks.find((t) => t.name === task_name)
		: null;
	...
	function populate_details_drawer(t) {
		...
		$("#list-detail-date").html(
			`<i class="fa fa-calendar"></i> ${
				t.exp_end_date ? frappe.datetime.str_to_user(t.exp_end_date) : "No Date"
			}`
		);
	...
	if (customer_input) {
		...
		$(customer_input).on("focus", function () {
			...
			frappe.call({
				...
				callback: function (r) {
					if (r.message) {
						frappe.pages["dak_task_list"].awesomplete_customer.list = r.message.map(
							(d) => d.value
						);
	...
	if (project_input) {
		...
		$(project_input).on("focus", function () {
			...
			frappe.call({
				...
				callback: function (r) {
					if (r.message) {
						frappe.pages["dak_task_list"].awesomplete_project.list = r.message.map(
							(d) => d.value
						);
	...
	if (activity_input) {
		...
		$(activity_input).on("focus", function () {
			frappe.call({
				...
				callback: function (r) {
					if (r.message) {
						frappe.pages["dak_task_list"].awesomplete_activity.list = r.message.map(
							(d) => d.value
						);
		...
		frappe.call({
			...
			callback: function (r) {
				if (!r.exc) {
					...
					frappe.pages["dak_task_list"].refresh_tasks(
						frappe.pages["dak_task_list"].page_wrapper
					);
				}
			},
...
frappe.pages["dak_task_list"].render_latest_task_card = function (task) {
	...
	// Check Timer State
	let is_running =
		frappe.pages["dak_task_list"].timer.running &&
		frappe.pages["dak_task_list"].timer.task_name === task.name;
    ...
    <div class="latest-task-card" style="margin-bottom: 0;">
        ...
        <div class="ltc-content">
            <div class="ltc-header">
                ...
                <span class="ltc-date"><i class="fa fa-calendar" style="margin-right: 5px;"></i> ${
					frappe.datetime.str_to_user(task.creation).split(" ")[0]
				}</span>
            </div>
        ...
        <div class="ltc-actions" style="flex-direction: column; align-items: flex-end; gap: 10px;">
             <div style="display: flex; gap: 15px;">
                 ...
                 <button onclick="frappe.pages['dak_task_list'].toggle_task_drawer(true, '${
						task.name
					}')" class="ltc-btn ltc-btn-view" title="Edit Task">
...
frappe.pages["dak_task_list"].start_clock_interval = function () {
	if (frappe.pages["dak_task_list"].timer.interval)
		clearInterval(frappe.pages["dak_task_list"].timer.interval);
			...
			frappe.call({
				...
				callback: function (r) {
					if (!r.exc && r.message) {
						...
						let task = frappe.pages["dak_task_list"].all_tasks.find(
							(t) => t.name === task_name
						);
