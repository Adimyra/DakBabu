// Ambient Declarations for Frappe and jQuery
...
interface Task {
    ...
    _status_color?: string;
    _border_color?: string;
    _bg_style?: string;
    _formatted_date?: string;
}
...
interface Page {
    main: any; // jQuery object
    set_title: (title: string) => void;
    set_indicator: (label: string, color: string) => void;
    add_menu_item: (label: string, action: () => void) => void;
    set_primary_action: (label: string, action: () => void) => void;
    current_view: string;
    refresh_data?: () => void;
    refresh?: () => void;
}
...
interface Window {
    open_task_dialog: (task_name: string) => void;
    dragged_card?: any;
}

frappe.pages['test'].on_page_load = function (wrapper: any) {
    var page: Page = frappe.ui.make_app_page({
        parent: wrapper,
        title: 'Test Page',
        single_column: true
    });
            ...
            callback: function (r: any) {
                ...
                if (r.message) {
                    ...
                    if (r.message.tasks) {
                        ...
                        r.message.tasks.forEach((t: Task) => {
                            // Calculate Overdue
                        ...
                        r.message.statuses.forEach((status: string) => {
                            r.message.kanban_columns.push({
                                title: status,
                                tasks: r.message.tasks.filter((t: Task) => t.status === status)
                            });
                        });
                    }
    ...
    page.main.on('click', '.task-edit-trigger', function (e: any) {
        e.preventDefault();
        var task_name = $(this).attr('data-task-name');
        if (task_name) {
            (window as any).open_task_dialog(task_name);
        }
    });

    // Event delegation for view switching
    page.main.on('click', '.btn-view-trigger', function (e: any) {
        e.preventDefault();
    ...
    // Initial Load
    if (page.refresh_data) page.refresh_data();
}

function get_status_attributes(status: string) {
...
function init_kanban(page: Page) {
    ...
    // Drag Start
    page.main.off('dragstart', '.kanban-card').on('dragstart', '.kanban-card', function (e: any) {
        const dt = e.originalEvent.dataTransfer;
        dt.setData('text/plain', $(this).attr('data-task-name'));
        dt.effectAllowed = 'move';
        $(this).addClass('dragging');
        (window as any).dragged_card = $(this); // Fallback for some browsers
    });

    // Drag End
    page.main.off('dragend', '.kanban-card').on('dragend', '.kanban-card', function (e: any) {
        $(this).removeClass('dragging');
        page.main.find('.kanban-column').removeClass('drag-over');
    });

    // Drag Over Column
    page.main.off('dragover', '.kanban-column').on('dragover', '.kanban-column', function (e: any) {
    ...
    // Drag Leave Column
    page.main.off('dragleave', '.kanban-column').on('dragleave', '.kanban-column', function (e: any) {
        $(this).removeClass('drag-over');
    });

    // Drop on Column
    page.main.off('drop', '.kanban-column').on('drop', '.kanban-column', function (e: any) {
        e.preventDefault();
        $(this).removeClass('drag-over');

        let task_name = e.originalEvent.dataTransfer.getData('text/plain');
        // Fallback if dataTransfer is empty
        if (!task_name && (window as any).dragged_card) {
            task_name = (window as any).dragged_card.attr('data-task-name');
        }
...
function create_customer_dialog(page: Page) {
...
function create_customer_address_step(page: Page, customer_name: string) {
...
function start_task_wizard(page: Page) {
    // Step 1: Customer
...
function step_2_project(page: Page, customer: string) {
    let d2 = new frappe.ui.Dialog({
...
function step_3_task(page: Page, customer: string, project: string) {
...
function step_4_description(page: Page, customer: string, project: string, basic_values: any) {
...
(window as any).open_task_dialog = function (task_name: string) {
                    ...
                    primary_action: (values: any) => {
                        frappe.call({
                            ...
                            callback: function (r: any) {
                                ...
                                // Refresh page
                                if (frappe.pages['test'] && (frappe.pages['test'] as any).refresh_data) {
                                    (frappe.pages['test'] as any).refresh_data();
                                }
                            }
                        });
                    }
