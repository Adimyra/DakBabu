frappe.pages["test"].on_page_load = function (wrapper) {
	...
	try {
					...
					frappe.db
						...
						.then((data) => {
							if (data && data.length > 0) {
								...
								$("#sphereTaskDesc").text(
									task.exp_end_date ? "Due: " + task.exp_end_date : "Latest Task"
								);
		...
		function renderListView(tasks) {
                ...
                <table class="frappe-list-table">
                    <thead>
                        <tr>
                            ...
                            ${
								state.visibleColumns.includes("Priority")
									? '<th style="width: 15%">Priority</th>'
									: ""
							}
                            ${
								state.visibleColumns.includes("Due Date")
			...
			if (tasks.length === 0) {
				html +=
					'<tr><td colspan="5" style="text-align:center; padding: 30px; color: #9ca3af;">No tasks found</td></tr>';
			} else {
				tasks.forEach((task) => {
					...
					if (state.visibleColumns.includes("Priority")) {
						html += `<td><span class="priority-text ${getPriorityClass(
							task.priority
						)}">${task.priority}</span></td>`;
					}
					if (state.visibleColumns.includes("Due Date")) {
						html += `<td><span class="${dateClass}">${
							task.exp_end_date || "-"
						}</span></td>`;
					}
		...
		function renderCardView(tasks) {
				...
				html += `
                  <div class="task-card" data-name="${task.name}">
                      <div class="task-card-header">
                          <span class="priority-text ${getPriorityClass(task.priority)}">${
					task.priority
				}</span>
			...
			if (tasks.length === 0)
				html =
					'<div style="text-align:center; padding: 30px; color: #9ca3af;">No tasks found</div>';
			$("#viewContainer").html(html);
		...
		function renderKanbanView(tasks) {
			...
			columns.forEach((col) => {
				...
				const colTasks = tasks.filter(
					(t) =>
						(t.status || "Open") === col ||
						(col === "Pending Review" && (t.status || "").includes("Pending"))
				);
				html += `
					...
					html += `
                     <div class="kanban-card" data-name="${task.name}">
                         ...
                         <div style="display:flex; justify-content:space-between; align-items:center;">
                             <span class="priority-text ${getPriorityClass(
									task.priority
								)}" style="font-size:11px;">${task.priority}</span>
                             <span style="font-size:11px; color:#6BA3BE;">${
									task.exp_end_date ? task.exp_end_date.slice(5) : ""
								}</span>
                         </div>
                     </div>`;
				});
		...
		function renderDispatch(tasks) {
			...
			// Bind Clicks (Generic)
			$wrapper
				.find(".task-item-row, .task-card, .kanban-card")
				.off("click")
				.on("click", function () {
		...
		$wrapper.find(".btn-new-task").on("click", function () {
			frappe.new_doc("Task");
		});
	} catch (e) {
		console.error("Test Page Error", e);
		$wrapper.prepend(
			'<div style="color:red; border:1px solid red; padding:20px;">ERROR: ' +
				e.message +
				"</div>"
		);
	}
