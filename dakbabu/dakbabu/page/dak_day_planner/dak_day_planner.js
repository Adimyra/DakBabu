frappe.pages['dak_day_planner'].on_page_load = function (wrapper) {
    ...
    $('#planner-start-time, #planner-end-time, #planner-interval').on('change', function () {
        frappe.pages['dak_day_planner'].config.start_hour = parseInt($('#planner-start-time').val());
...
frappe.pages['dak_day_planner'].render_pool = function () {
    ...
    if (!frappe.pages['dak_day_planner'].tasks || frappe.pages['dak_day_planner'].tasks.length === 0) {
        container.html('<div style="text-align: center; color: #9ca3af; margin-top: 30px;">No tasks due for today.</div>');
        return;
    }
    ...
    frappe.pages['dak_day_planner'].tasks.forEach(task => {
        ...
        let html = `
            <div class="planner-task-card" draggable="true" data-task="${encodeURIComponent(JSON.stringify(task))}" id="pt-${task.name}">
...
frappe.pages['dak_day_planner'].render_grid = function () {
    ...
    $('.time-slot-dropzone').on('drop', function (e) {
        ...
        try {
            ...
            frappe.show_alert({ message: `Scheduled ${task.subject} at ${slotTime}`, indicator: 'blue' }, 2);
